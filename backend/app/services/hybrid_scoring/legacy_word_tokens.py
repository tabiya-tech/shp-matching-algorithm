...
The functions here are self-contained — no imports from ``text_builders``,
``bm25library``, or anywhere else in ``hybrid_scoring`` — so the file can be
safely deleted in a future cleanup without disturbing the live pipeline.

Nothing else in ``hybrid_scoring`` imports from this module.
"""
