...
def _fmt_stats(xs):
    ...
    if not xs:
        return "n/a"
    return (f"mean={stats.mean(xs):.3f} sd={ (stats.pstdev(xs) if len(xs)>1 else 0.0):.3f} "
...
def analyse_diversity(users, rec_key, label_key, title, k, report):
    ...
    sect = {
        ...
        "coverage_distinct_over_slots": round(distinct / total_slots, 3) if total_slots else None,
        "mean_pairwise_jaccard_topk": round(_mean_pairwise_jaccard(topk_sets), 3),
        "hhi": round(sum((c / total_slots) ** 2 for c in counts), 4) if total_slots else None,
        "gini_of_appearance_counts": round(_gini(counts), 3),
        "normalised_entropy": round(_norm_entropy(counts), 3),
        "most_recommended": [
            {
                "id": iid, "label": label_of.get(iid, iid),
                "n_users_topk": c, "pct_users": round(100 * c / n_users, 1),
                "mean_rank": round(stats.mean([x for x in rank_when_appears[iid] if x]), 1),
            }
            for iid, c in most
        ],
    ...
    print(f"  users={n_users}  distinct items in top-{k}={distinct}  "
          f"(of {total_slots} slots → coverage {sect['coverage_distinct_over_slots']})")
    print(f"  mean pairwise Jaccard of top-{k} sets: {sect['mean_pairwise_jaccard_topk']}  "
          f"(0=all different, 1=identical)")
    print(f"  concentration: HHI={sect['hhi']}  Gini={sect['gini_of_appearance_counts']}  "
    ...
    if ubiquitous:
        ...
        for iid, c in ubiquitous[:10]:
            print(f"      {100*c/n_users:5.1f}% ({c}/{n_users})  {label_of.get(iid, iid)}")
    print(f"  most-recommended {title.lower()}:")
    for m in sect["most_recommended"][:8]:
        print(f"      {m['pct_users']:5.1f}% ({m['n_users_topk']}/{n_users})  "
...
def analyse_balance(users, report):
    print("\n=== 2. SKILLS ↔ PREFERENCES BALANCE ===")
    print("  (final_score = combine(u_hat, p_hat); u_hat = preference utility, "
    ...
    for rec_key, _label, title in (OPP, OCC):
        ...
        for u in users:
            ...
            u_all += us
            p_all += ps
            f_all += [r.get("final_score") for r in recs if r.get("final_score") is not None]
            if len(us) > 1:
        ...
        driver = "u_hat (preferences)" if mean_u_sd > mean_p_sd else "p_hat (skills/cosine)"
        compressed = []
        ...
        print(f"    within-user spread → u_hat sd={mean_u_sd:.4f}  p_hat sd={mean_p_sd:.4f}  "
              f"→ ranking driven by {driver}")
        print(f"    binding (smaller) factor: u_hat in {binding_u}, p_hat in {binding_p} of recs")
        if compressed:
            print(f"    ⚠ LOW SPREAD in {', '.join(compressed)} (range < 0.10) — rankings barely "
                ...
                if ec is not None:
                    ...
                    if (r.get("rank") or 1e9) <= k and ec < min_ess_cos:
                        ...
                        if len(low_examples) < 30:
                            low_examples.append({
                                "user_id": u.get("user_id"), "rank": r.get("rank"),
                                "label": r.get(label_key), "essential_cos": round(ec, 3),
                                "u_hat": _sb(r, "u_hat"), "p_hat": _sb(r, "p_hat"),
                                "final_score": r.get("final_score"),
                            })
        corr = None
        if len(rank_ess_pairs) > 2:
            try:
                corr = round(stats.correlation([p[0] for p in rank_ess_pairs],
                                               [p[1] for p in rank_ess_pairs]), 3)  # py3.10+
            except Exception:
                corr = None
        bin_mean = stats.mean(binary_fits) if binary_fits else None
        sect = {
            "binary_essential_fit_mean": round(bin_mean, 3) if bin_mean is not None else None,
            "binary_essential_fit_saturated": bool(bin_mean is not None and bin_mean > 0.99),
            ...
            "low_essential_cos_examples": sorted(low_examples, key=lambda e: e["essential_cos"]),
        }
        report.setdefault("achievability", {})[rec_key] = sect
        print(f"\n  {title}:")
        print(f"    binary essential_fit (share of essential skills meeting threshold): mean="
              f"{sect['binary_essential_fit_mean']}, eligible={sect['pct_eligible']}%")
        if sect["binary_essential_fit_saturated"]:
            print("      ⚠ SATURATED at ~1.0 — the meets_threshold gate passes virtually every "
                  "essential skill, so binary eligibility is uninformative. Use the continuous cosine ↓")
        print(f"    essential cosine (continuous, mean best-match per essential skill): "
              f"{sect['essential_cosine_continuous']}")
        print(f"    rank↔essential_cosine corr: {corr}  (≈0 ⇒ ranking is NOT driven by how well "
              f"the user's skills fit; recs can be preference-led, not skill-achievable)")
        print(f"    ⚠ {topk_low} top-{k} recs with essential cosine < {min_ess_cos} "
              f"(weak skill fit despite being recommended). Weakest examples:")
        for e in sect["low_essential_cos_examples"][:10]:
            print(f"        {e['user_id'][:12]}.. rank{e['rank']:<3} ess_cos={e['essential_cos']} "
...
def _explain_pair(user, rec, rec_key, label_key, user_skills):
    ...
    print(f"    final_score={rec.get('final_score')}  u_hat(pref)={_sb(rec,'u_hat')}  "
          f"p_hat(skills cos)={_sb(rec,'p_hat')}  total_skill_utility={_sb(rec,'total_skill_utility')}")
    print(f"    is_eligible={rec.get('is_eligible')}  essential_fit={fit if fit is None else round(fit,2)} "
          f"({met}/{total} essential skills met)  demand={_sb(rec,'demand_label')}")
    if user_skills:
        print(f"    user's top skills: {user_skills[:240]}")
    em = sorted((rec.get("matched_skills") or {}).get("essential_skill_matches") or [],
                key=lambda m: float(m.get("similarity") or 0), reverse=True)
    unmet = [m for m in em if not m.get("meets_threshold")]
    print(f"    essential skills MET ({total-len(unmet)}): " +
          ", ".join(f"{m.get('job_skill_label')}~{m.get('best_user_skill_label')}({m.get('similarity')})"
                    for m in em if m.get("meets_threshold"))[:400] or "(none)")
    print(f"    essential skills NOT met ({len(unmet)}): " +
          (", ".join(str(m.get("job_skill_label")) for m in unmet[:25]) or "(none)"))
    prefs = rec.get("matched_preferences") or []
    if prefs:
        print("    matched preferences: " +
              ", ".join(f"{p.get('attribute')}={p.get('job_value_label') or p.get('job_value')}"
                        f"{'✓' if p.get('matched') else '✗'}" for p in prefs))
...
def explain(users, token, run_dir, k):
    ...
    if token in by_id:
        ...
        for rec_key, label_key, title in (OPP, OCC):
            print(f"\n  {title}:")
            for r in sorted(u.get(rec_key) or [], key=lambda r: r.get("rank") or 1e9)[:k]:
                met, total, fit = _essential_fit(r)
                print(f"    rank{r.get('rank'):<3} final={r.get('final_score')} "
...
def main():
    ...
    ap.add_argument("--top-k", type=int, default=5, help="K for diversity/top-K checks (default 5)")
    ap.add_argument("--explain", metavar="TOKEN", default=None,
                    help="Explain a user_id, an item uuid, or a title/label substring")
    ap.add_argument("--user", default=None, help="With --item, explain one (user, item) pair")
    ap.add_argument("--item", default=None, help="Item uuid or title/label substring (use with --user)")
    ap.add_argument("--out", default=None, help="Write the JSON report here (default: <run_dir>/analysis_report.json)")
    args = ap.parse_args()
    ...
    if args.user and args.item:
        ...
        for rec_key, label_key, _t in (OPP, OCC):
            for r in u.get(rec_key) or []:
                if args.item in {str(r.get("uuid")), str(r.get("originUuid"))} or il in str(r.get(label_key) or "").lower():
